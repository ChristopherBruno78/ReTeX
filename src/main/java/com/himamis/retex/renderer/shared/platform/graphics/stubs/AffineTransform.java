...
import com.himamis.retex.renderer.shared.platform.graphics.Transform;
 ...
 *
 * <p>Such a coordinate transformation can be represented by a 3 row by 3 column matrix with an
 * implied last row of [ 0 0 1 ]. This matrix transforms source coordinates {@code (x,y)} into
 * destination coordinates {@code (x',y')} by considering them to be a column vector and multiplying
 * the coordinate vector by the matrix according to the following process:
 *
 * <pre>
 ...
 *
 * <p>In some variations of the <code>rotate</code> methods in the <code>AffineTransform</code>
 * class, a double-precision argument specifies the angle of rotation in radians. These methods have
 ...
 *
 * <p>Since 90 degrees is represented as <code>PI/2</code> in radians, and since PI is a
 * transcendental (and therefore irrational) number, it is not possible to exactly represent a
 * multiple of 90 degrees as an exact double precision value measured in radians. As a result it is
 * theoretically impossible to describe quadrant rotations (90, 180, 270 or 360 degrees) using these
 * values. Double precision floating point values can get very close to non-zero multiples of <code>
 * PI/2</code> but never close enough for the sine or cosine to be exactly 0.0, 1.0 or -1.0. The
 * implementations of <code>Math.sin()</code> and <code>Math.cos()</code> correspondingly never
 ...
 *
 * <p>The net result of these issues is that if the <code>Math.sin()</code> and <code>Math.cos()
 * </code> methods are used to directly generate the values for the matrix modifications during
 * these radian-based rotation operations then the resulting transform is never strictly
 * classifiable as a quadrant rotation even for a simple case like <code>rotate(Math.PI/2.0)</code>,
 * due to minor variations in the matrix caused by the non-0.0 values obtained for the sine and
 * cosine. If these transforms are not classified as quadrant rotations then subsequent code which
 * attempts to optimize further operations based upon the type of the transform will be relegated to
 * its most general implementation.
 *
 * <p>Because quadrant rotations are fairly common, this class should handle these cases reasonably
 ...
 * approximately 0.0000000211 radians (or 0.00000121 degrees) around small multiples of <code>
 * Math.PI/2.0</code>.
 *
 * @author Jim Graham
 * @since 1.2
...
public class AffineTransform implements Transform {
  ...
  /**
   * Constructs a new <code>AffineTransform</code> that is a copy of the specified <code>
   ...
   *
   * @param flatmatrix the double array containing the values to be set in the new <code>
   *     AffineTransform</code> object. The length of the array is assumed to be at least 4. If the
   *     length of the array is less than 6, only the first 4 values are taken. If the length of the
   *     array is greater than 6, the first 6 values are taken.
   * @since 1.2
   */
   ...
   *
   * <p>This operation is equivalent to the following sequence of calls:
   *
   * <pre>
   * AffineTransform Tx = new AffineTransform();
   ...
   * @since 1.6
   */
  public static AffineTransform getQuadrantRotateInstance(
      int numquadrants, double anchorx, double anchory) {
    AffineTransform Tx = new AffineTransform();
   ...
   * the vector pointing from the origin to the specified vector coordinates. If both <code>vecx
   * </code> and <code>vecy</code> are 0.0, the transform is set to an identity transform. This
   * operation is equivalent to calling:
   *
   * <pre>
   ...
   *
   * @return <code>true</code> if this <code>AffineTransform</code> is an identity transform; <code>
