...
package com.himamis.retex.renderer.shared.cache;
...
public final class JLaTeXMathCache {
  ...
  private static ConcurrentMap<CachedTeXFormula, SoftReference<CachedImage>> cache =
      new ConcurrentHashMap<CachedTeXFormula, SoftReference<CachedImage>>(128);
  ...
  public static void setMaxCachedObjects(int max) {
    ...
    cache.clear();
    cache =
        new ConcurrentHashMap<CachedTeXFormula, SoftReference<CachedImage>>(JLaTeXMathCache.max);
  }
  ...
  public static int[] getCachedTeXFormulaDimensions(
      String f, int style, int type, int size, int inset, Color fgcolor) throws ParseException {
  ...
  private static SoftReference<CachedImage> makeImage(CachedTeXFormula cached)
    ...
    Image image =
        new Graphics().createImage(icon.getIconWidth(), icon.getIconHeight(), Image.TYPE_INT_ARGB);
    ...
    SoftReference<CachedImage> img =
        new SoftReference<CachedImage>(new CachedImage(image, cached), queue);
