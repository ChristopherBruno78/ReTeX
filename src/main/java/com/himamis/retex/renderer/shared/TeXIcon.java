...
import com.himamis.retex.renderer.shared.TeXConstants.Align;
...
/**
 * An {@link Icon} implementation that will paint the TeXFormula that created it.
 *
 * <p>This class cannot be instantiated directly. It can be constructed from a TeXFormula using the
 * {@link TeXFormula#createTeXIcon(int,double)} method.
 *
...
public class TeXIcon implements Icon {
  ...
  /**
   * Change the width of the TeXIcon. The new width must be greater than the current width,
   * otherwise the icon will remain unchanged. The formula will be aligned to the left ({@linkplain
   * TeXConstants#ALIGN_LEFT}), to the right ({@linkplain TeXConstants#ALIGN_RIGHT}) or will be
   * centered in the middle ({@linkplain TeXConstants#ALIGN_CENTER}).
   *
  ...
  /**
   * Change the height of the TeXIcon. The new height must be greater than the current height,
   * otherwise the icon will remain unchanged. The formula will be aligned on top
   * (TeXConstants.TOP), at the bottom (TeXConstants.BOTTOM) or will be centered in the middle
   * (TeXConstants.CENTER).
   *
