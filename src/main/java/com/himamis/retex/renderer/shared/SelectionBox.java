...
public class SelectionBox extends Box {
  ...
  public void draw(Graphics2DInterface g2, double x, double y) {
    ...
    g2.setStroke(FactoryProvider.getInstance().getGraphicsFactory().createBasicStroke(1, 0, 0, 1));
    ...
    SelectionBox.endX =
        transform.getScaleX() * (x + content.width)
            + transform.getShearX() * (y + content.depth)
            + transform.getTranslateX();
    SelectionBox.endY =
        transform.getScaleY() * (y + content.depth)
