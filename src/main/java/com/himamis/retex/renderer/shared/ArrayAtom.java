  ...
  public Box createBox(TeXEnvironment env) {
    ...
    for (int i = 0; i < row; ++i) {
      ...
      for (int j = 0; j < col; ++j) {
        ...
        final Box b = (at == null) ? StrutBox.getEmpty() : at.createBox(env);
        boxarr[i][j] = b;
    ...
    for (final MulticolumnAtom multi : listMulti) {
      ...
      for (int j = c + 1; j < N; ++j) {
        w += colWidth[j] + hseps[2 * j] + hseps[2 * j + 1] + separatorBoxes.get(j + 1).getWidth();
      }
    ...
    for (int j = 0; j < col; ++j) {
      matW += colWidth[j] + hseps[2 * j] + hseps[2 * j + 1] + separatorBoxes.get(j).getWidth();
    }
