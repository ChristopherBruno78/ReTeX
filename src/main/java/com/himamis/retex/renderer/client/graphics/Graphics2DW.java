 ...
 */
package com.himamis.retex.renderer.client.graphics;

import com.himamis.retex.renderer.client.DrawingFinishedCallback;
import com.himamis.retex.renderer.client.font.AsyncLoadedFont;
import com.himamis.retex.renderer.client.font.AsyncLoadedFont.FontLoadCallback;
import com.himamis.retex.renderer.client.font.DefaultFont;
import com.himamis.retex.renderer.client.font.FontW;
import com.himamis.retex.renderer.client.font.FontWrapper;
...
import com.himamis.retex.renderer.shared.platform.graphics.stubs.AffineTransform;
...
public class Graphics2DW implements Graphics2DInterface {
  ...
  private void initFont() {
    font =
        new DefaultFont(
            context.getFont(), Font.PLAIN, (int) Math.round(FontLoader.PIXELS_PER_POINT));
  }
  ...
  @Override
  public void fill(com.himamis.retex.renderer.shared.platform.geom.Shape s) {
    context.fill(s);
  }
  ...
  private void drawRoundRectangle(double x, double y, double width, double height, double radius) {
    context.beginPath();
  ...
  private static class FontDrawContext {
    private final Graphics2DW graphics;
    private final String text;
    private final int x;
    private final int y;

    private final AffineTransform transformCopy;
    private final FontW font;
    private final ColorW color;
  ...
  public void drawText(String text, int x, int y) {
    if (!font.isLoaded()) {
      ...
      font.addFontLoadedCallback(
          new FontLoadCallback() {
  ...
  /**
   * Applies the transformation matrix to the context. Please ensure one call to graphics.save()
  ...
  /**
   * Sets the transformation matrix. Please ensure one call to graphics.save() before this method
   * and graphics.restore() after this method.
   *
   * @param transform transformation matrix
   */
