 ...
 */
package com.himamis.retex.renderer.client.font.opentype;
...
public class OpentypeFont extends FontW implements OpentypeFontStatusListener {
  ...
  private final Opentype opentype = Opentype.INSTANCE;
  ...
  public Shape getGlyphOutline(FontRenderContext frc, CharFont cf) {
    ...
    FontGlyph outline = wrap.getGlyphOutline(cf + "", this.getSize());

    double height = cf.fontInfo.getHeight(cf.c);
    double width = cf.fontInfo.getWidth(cf.c);

    Rectangle2D rect =
        FactoryProvider.getInstance().getGeomFactory().createRectangle2D(0, 0, width, height);
