 ...
 * define their type, used for determining what glue to use between adjacent atoms in a "row
 * construction". That can be one single type by asigning one of the type constants to the {@link
 * #type} field. But they can also be defined as having two types: a "left type" and a "right type".
 * This can be done by implementing the methods {@link #getLeftType()} and {@link #getRightType()}.
 * The left type will then be used for determining the glue between this atom and the previous one
 * (in a row, if any) and the right type for the glue between this atom and the following one (in a
 * row, if any).
 *
...
public abstract class Atom {
  ...
  /**
   * Get the type of the rightermost child atom. Most atoms have no child atoms, so the "left type"
   * and the "right type" are the same: the atom's type. This also is the default igmplementation.
   * But Some atoms are composed of child atoms put one after another in a horizontal row. These
   * atoms must override this method.
   *
